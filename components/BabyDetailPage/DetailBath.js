...
function DetailBath() {
    ...
    useEffect(() => {
        fetch(`https://baby-note-backend.vercel.app/babyData/${user.babies[0]._id}/Care`)
    ...
    const handleSave = () => {
        ...
        fetch(`https://baby-note-backend.vercel.app/babyData/care/${selectedId}`, {
            method: "PUT",
    ...
    .map((data, i) => {
      ...
      return <Care key={i} date={date} image={'/bain.svg'} onClickSelect={() => handleOpenModal(data._id)}/>;
    });
